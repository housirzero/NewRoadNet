...
public class RouteRec {
	...
	private static String select_mode = "-1"; // 所选模式

	private static Set<Integer> modeSet = null;
...
//		regionSet = CalProbability.statZonePassFlow(FilePath.dataFolder + "RouteRec\\TripChain\\20150104_TripChain3.csv", 0);
...
//		Integer[] Ds = new Integer[]{67, 69, 70, 75, 469, 470, 478, 480,
//				482, 501, 502, 503, 514, 924, 931, 952, 953, 954};
		Integer[] Ds = new Integer[]{157};
		Set<Integer> DSet = new HashSet<Integer>(Arrays.asList(Ds));
		try {
//			calMode(DSet, CalProbability.timeRegionIndex, 0);
			modeSet = new HashSet<Integer>(Arrays.asList(new Integer[]{1, 2, 3}));
			select_mode = "123";
			calMode(DSet, CalProbability.timeRegionIndex);
			modeSet = new HashSet<Integer>(Arrays.asList(new Integer[]{3}));
			select_mode = "3";
			calMode(DSet, CalProbability.timeRegionIndex);
		} catch (IOException e) {
			e.printStackTrace();
	...
	// 开始按选定时段和模式统计
	public static void calMode(Set<Integer> DSet, int timeRegionIndex) throws IOException{
		bw = new BufferedWriter(new FileWriter(FilePath.dataFolder + 
				String.format("mode_time\\mode%s_time%s_%s.csv", select_mode, timeRegionIndex, "D157")));
	...
	public static void randomTime(int D){
		...
		for(int O=1;O< regionNum+1 ;O++){
			...
			if(D!=O)
			{
				System.out.println(O + "------------------------------------>" + D + " start");
				getResult(O, D, timeSliceNum*timeSlice-1);
			}
		}
	}
